 ...
 */
static char in_comment = 0; /* non-zero while inside skip_comment() */
...
char *lookup_macro(void )
{
	...
	for(unsigned int i=0; i < define_top; ++i) {
...
/* lookup token in token table */
	...
	for(unsigned int i=0; (ptr = tokens[i]); ++i) {	/* look through entire table */
...
/* Handle concatinated strings */
		...
		--input_pos;
		if(fold) {
			for(unsigned int i=0; i < gvalue; ++i) {
				ptr = literal_pool+i;
				last_pos = literal_pool + gvalue;
				unsigned int j = literal_top - gvalue;
				while(*ptr++ == *last_pos++) {
					if(!--j) {
						literal_top = gvalue;
						gvalue = i;
...
/* If end of file, back up through include files */
	if(get_lin(input_pos = input_line)) {	/* end of file encountered */
		if(file_depth) {
			...
			/* If a block comment scanner is active, inject the close
			 * sequence so it terminates at the file boundary.
			 * Standard C: comments may not span #include boundaries. */
			if(in_comment) {
				input_line[0] = '*';
				input_line[1] = '/';
				input_line[2] = 0;
...
/* handle '#' preprocessor directives */
	if(match("#define")) {					/* define statement */
		...
		char *name = parse();
		for(unsigned int i = 0; i < define_top; i += 2) {
...
bool declare_arg(void)
{
	...
	if(token == SYMBOL) {
		for(unsigned int i = 0; i < td_top; ++i) {
			if(equal_string(gsymbol, td_name[i])) {
				unget_token(token);
...
{
	for(unsigned int i=0; i < global_top; ++i)
		if(equal_string(gsymbol, s_name[i]))
			return s_type[sptr = i] |= REFERENCE;
	return 0;
}
