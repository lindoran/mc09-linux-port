 ...
 */
#define	INCLUDE_DIR	"./include"	/* default include directory (Linux) */
#define	DIR_END		"/"		/* path separator (Linux) */
...
/*
 * Minimal formatted print into output_ptr buffer.
 * Formats: %s (string), %u (unsigned), %2u (2-digit 0-padded).
 */
static void fprint(char *fmt, ...)
{
	char tmp[32];
	char *s;
	unsigned v;
	...
	if((i = lookup_macro(0)) != -1) {	/* Substitution required */
		...
		while((c = *input_ptr)) {	/* copy over definition */
			...
			if(c == '"') {
				...
				continue; }
			if(c == '#' && *(input_ptr+1) == '#') {
				/* ## token paste: strip whitespace and concatenate */
				input_ptr += 2;
				while(output_ptr > save_ptr &&
